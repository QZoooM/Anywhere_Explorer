...
#--方块大类
class Blocks(object):
    def __init__(self,x,y,z,surf,l_x=1,l_y=1,l_z=1): #48x48,46*32;x-->width,y-->height,z-->depth
        self.x = x
        self.y = y
        self.z = z
        self.surf = surf
...
class Entity(object):
    basicTimeCount = 0
    def __init__(self,x,y,z,width,height,up,surf,type,life,speed=1,faceleft=0): #Whole:27x45;Head:27x21;Body:27x24.
        self.x = self.tx = x
        self.y = y
        self.ty = -999
        self.z = self.tz = z
...
class Savings(object):
    def __init__(self,FolderName,name,ChTime,T=[],F=[],D=[],O=[],G=[],I=[],M=[],ChunckFilesList=[]):#区块数量
        ...
        self.I = I
        self.M = M
        self.ChunckFilesList = ChunckFilesList
    def scan(self): #扫描存档
        menurd.SavPhs.clear()
        menurd.SavFds.clear()
        ...
        with open("saveport/" + GameVar.sav.FolderName + "/Ck/T000.ck") as ck:
            ...
            for line in ck.readlines():
                ...
                if line.find("chunck") > -1:
                    ...
                    continue
                try:
                    exec("self.Chuncks[tempChunckListNum].blocks.append(block(" + line +"))")
