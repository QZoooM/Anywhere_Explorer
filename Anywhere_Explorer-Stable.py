...
class Renderbase(object):
    ...
    #时间计算使用的‘上一次时间’
    RenderLastTime = 0
    RenderInterval = 1/(basefam+2)
    LoadLastTime = 0
    LoadInterval = EnTkSpd/maxtick/tickspeed
#基础音频信息
class Ifpl(object):
    def __init__(self):
        self.pl = 1
...
class Blocks(object): #48x48,46*32;x-->width,y-->height,z-->depth
    def __init__(self,x,y,z,surf,l_x=48,l_y=48,l_z=48):
        self.x = x
        self.y = y
        self.z = z
        self.surf = surf
...
class Chars(object): #Whole:27x45;Head:27x21;Body:27x24.
    def __init__(self,x,y,z,width,height,up,surf,type,life,speed=1,faceleft=0,v_y=0):
        self.x = x
        self.y = y
        self.z = z
        self.width = width
        ...
        self.type = type
        self.life = life
        self.upab = self.dwab = self.lfab = self.rtab = 0
        self.speed = speed
    ...
    def paint(self):
        ...
        window.blit(self.surf[0],(paint_x,Renderbase.window_y - self.z*32 - self.y*48 - self.height + 1))
    def gravbox(self): #y轴碰撞判定
        if self.y > 1:
            self.felling = 1
        if self.felling == 1:
            if self.y == 1:
        ...
        #   if self.y == recenteBlock.y+1:
        #      self.felling = 0
        #   if self.y < recenteBlock.y+1:
        #      self.v_z = 0
        #      self.y = recenteBlock.y+1
        pass
    def move(self):
        if self.felling == 1:
            if Times_up(PhyAgri.PhyLastTime,PhyAgri.PhyInterval):
        ...
        if self.rtab == 1:
            self.x += 0.02
        #print((round(self.x),round(self.y),round(self.z))) #For test
...
#地图类
class World(object):
    def __init__(self,T_x,T_y,T_z,LogicType,Blocks):
        self.T_x = T_x
        self.T_y = T_y
        self.T_z = T_z
...
class Block(Blocks):
    def __init__(self,x,y,z,width,height,depth,type,):
        if type == 0:
            surf = Stone_img[0]
        Blocks.__init__(self,x,y,z,width,height,depth,surf)
...
def render_window():
    #此if便于停止
    if not Times_up(Renderbase.RenderLastTime,Renderbase.RenderInterval):
        return
    Renderbase.RenderLastTime = time.time()
    if GameVar.wait == 0:
        renderBG()
        renderingame()
        renderelement()
...
#加载总线
def Load_():
    if not Times_up(Renderbase.LoadLastTime,Renderbase.LoadInterval):
        return
    Renderbase.LoadLastTime = time.time()
    #碰撞侦测
    check_hit()
    #组件移动
    element_move()
